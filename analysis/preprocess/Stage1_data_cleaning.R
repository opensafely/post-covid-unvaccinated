...
stage2 <- function(group) {
  ...
  cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), 0 ,"Study defined sample size after QA checks")
  ...
  cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), as.numeric(cohort_flow[1,1]) - nrow(input),"Criteria 1 (Inclusion): Alive on the first day of follow up") # Feed into the cohort flow
  
  # Inclusion criteria 2: Known age between 18 and 110 on 01/01/2020 
  
  input <- input %>% 
    filter(cov_num_age >= 18 & cov_num_age <= 110)
  cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), as.numeric(cohort_flow[2,1]) - nrow(input), "Criteria 2 (Inclusion): Known age between 18 and 110 on 01/01/2020") # Feed into the cohort flow
  
  # Inclusion criteria 3: Known sex
  
  input <- input %>% 
    filter(cov_cat_sex == "Male" | cov_cat_sex == "Female")
  cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), as.numeric(cohort_flow[3,1]) - nrow(input), "Criteria 3 (Inclusion): Known sex")
  
  # Inclusion criteria 4: Known deprivation 
  
  input <- input %>% 
    drop_na(cov_cat_deprivation)
  cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), as.numeric(cohort_flow[4,1]) - nrow(input), "Criteria 4 (Inclusion): Known deprivation")
  ...
  cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), as.numeric(cohort_flow[5,1]) - nrow(input), "Criteria 5 (Inclusion): Registered in an English GP with TPP software for at least 6 months prior to the study start date")
  ...
  cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), as.numeric(cohort_flow[6,1]) - nrow(input), "Criteria 6 (Inclusion): Known region")
  ...
  if (group == "diabetes"){
    ...
    cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input),as.numeric(cohort_flow[7,1]) - nrow(input), "Diabetes specific criteria: Remove those with diabetes prior to study start date")
    ...
    cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), as.numeric(cohort_flow[7,1]) - nrow(input), "Gestational diabetes: The study population will be restricted to women.")
    
  } else if (group == "mental_health"){
    # Mental health analyses exclusion criteria
    input <- input %>% 
      filter()
    cohort_flow[nrow(cohort_flow)+1,] <- c(nrow(input), as.numeric(cohort_flow[7,1]) - nrow(input), "Mental health: .")
  }
