...
follow_up_end_dates <- function(group){
  ...
  for(event in active_analyses$outcome_variable){
    ...
    # We want to be able to include follow up time prior to exposure for the pheno of interest which uses date_expo_censor
  ...
  saveRDS(input, paste0("output/follow_up_end_dates_",group,".rds"))
}

# Run function on active outcome groups
