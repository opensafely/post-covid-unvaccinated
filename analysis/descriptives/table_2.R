...
## Date:     28 Feb 2022
##
## Data:     Post covid project study population
##
...
table_2_subgroups_output <- function(group){
  ...
  survival_data <- read_rds(paste0("output/input_stage1_",group,".rds"))
  ...
  for(i in 1:nrow(analyses_of_interest)){
    ...
    print(paste0("event count and person years have been produced successfully for", analyses_of_interest$event[i], " in ", group, " population!"))
  }
...
table_2_calculation <- function(survival_data, event, cohort, subgroup, stratify_by, stratify_by_subgroup){
  ...
  #Add age/sex calculations - require total values and not for subgroups (i.e. subgroup = "MAIN")
  for(l in c("Female","Male")){
    for(m in agelabels){
      age_low <- gsub("_.*$","",m)
      age_high <- gsub(".*_","",m)
      ...
      assign(paste(l,"_",m,"_unexposed_days",sep=""), round(sum(data_active$person_days_unexposed[data_active$sex==l & data_active$cov_num_age >= age_low & data_active$cov_num_age <= age_high],na.rm = TRUE)))
      ...
      assign(paste(l,"_",m,"_exposed_days",sep=""), round(sum(data_active$person_days_exposed[data_active$sex==l & data_active$cov_num_age >= age_low & data_active$cov_num_age <= age_high],na.rm = TRUE)))
    }
  }
