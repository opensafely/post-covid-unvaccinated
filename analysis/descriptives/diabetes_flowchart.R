...
diabetes_flow_function <- function(group) {
  # Load Stage 1 dataset
  
  diabetes_df <-read_rds(paste0("output/input_stage1_",group,".rds"))
  ...
  write.csv(values_df_t, file = paste0("output/review/figure-data/diabetes_flow_values_",group,".csv")) # save
